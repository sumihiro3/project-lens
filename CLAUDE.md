...
7. **振り返り(retrospective)** — サイクル全体を振り返り、新発見・知見は Obsidian Vault の `knowledge/` に、**mistake(失敗)があれば `mistakes/YYYY-MM-DD-<topic>.md` に「原因と対策のペア」で**記録する(両者を区別する)。サイクル末にメインのループ(全文脈を持つ)が実施し、各ワークフローが集めた「判断に迷った点」と review/QA/実機フィードバックの指摘を入力にする。「mistake は無かったか?」を必ず明示的に問う(知見だけ残して失敗を残さないのを防ぐ)

ワークフローは `.claude/workflows/`、コマンドは `.claude/commands/` にある。
振り返り(ステップ7)はワークフローではなくメインのループの責務(会話を跨いだ手戻りはワークフロー内エージェントの単一フェーズ文脈では拾えないため)。
