...
- サイドバーから「レポート」ページへアクセス可能。全ワークスペース・全プロジェクトの課題統計を集約
- **横断サマリー**: 未完了・期限超過・停滞・自分担当の要対応件数とAIリスク分布を統計テーブルに表示。統計テーブルの上に**優先対応リスト**（横断上位N件のフラット一覧 + プロジェクト別アコーディオン）を主役として表示し、各課題には理由チップ（期限超過/リスク高・中/停滞/未割当）を色分けして付与。行クリックでBacklogの課題ページをブラウザで開く。テーブル下部には**AI構造化インサイトカード**（概況2〜3文 + 推奨アクション1〜2文）を表示
- **週次/月次アクティビティ**: ISO週（`YYYY-Www`）または月（`YYYY-MM`）単位で、プロジェクト別の新規作成・更新・完了件数を集計。期間セレクタで過去レポートへの切り替えが可能
- 統計はSQLで決定的に算出（LLM不使用）。narrativeのみAI生成で、AI非対応環境では統計のみ表示に縮退
- プロジェクト行をクリックすると、そのプロジェクトに絞り込んだ課題一覧へ遷移

### 🗂️ 課題の背景・経緯要約

- 課題詳細ダイアログに「背景・経緯を要約」ボタンを追加
- コメント履歴からAIが主要な経緯・決定事項・文脈をダイジェストとして生成
- コンテンツハッシュによるキャッシュ付き（2回目以降は即返し）。コメントなし・AI非対応時は空表示に縮退
...
- サイドバーの「レポート」からレポートページを開く
- **横断サマリー**タブ: 全プロジェクト横断で未完了・期限超過・停滞・自分担当件数とAIリスク分布を表示。統計テーブルの上に**優先対応リスト**（横断フラット一覧 + プロジェクト別アコーディオン）が表示され、理由チップ・担当者が確認できる。行クリックでBacklogの課題ページへ遷移。テーブル下部の**AI構造化インサイトカード**で概況と推奨アクションを確認できる
- **週次/月次アクティビティ**タブ: 選択した期間のプロジェクト別 新規作成・更新・完了件数を表示。期間セレクタで過去のレポートへ切り替え可能
- レポートはバックグラウンドで1日1回自動生成。レポートページから手動生成も可能

### 課題の背景・経緯要約

- 任意の課題をクリックして詳細ダイアログを開く
- 「背景・経緯を要約」ボタンをクリックすると、AIがコメント履歴を分析し、経緯・決定事項・文脈を要約表示
- 結果はキャッシュされるため、2回目以降は即座に表示される
