    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::block_on(async move {
                ...
                let options = SqliteConnectOptions::from_str(&db_url).expect("failed to parse db url")
                    .create_if_missing(true);

                // データベースクライアントを作成してアプリケーション状態に登録
                let db_client = db::DbClient::new_with_options(options).await.expect("failed to init db client");
