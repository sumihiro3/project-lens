...
スキーマは以下と一致させること:
- `src-tauri/src/ai/foundation_models.rs` の `SidecarRequest` / `SidecarResponse`（プロトコル本体の enum。`analyze` / `embed` / `summarize` / `availability` / `shutdown` のすべてを定義）
- `src-tauri/src/ai/mod.rs` の `AiAnalysisInput` / `AiAnalysisOutput`（`analyze` の入出力型）
- `src-tauri/src/ai/embedding.rs` の `EmbeddingInput` / `EmbeddingOutput` / `EmbedPrefix`（`embed` の入出力型）
...
| `summarize`    | `instruction`, `context`(省略可), `lang`(`ja`/`en`)                                          | 自由文生成 (v0.4.6)。`context` は呼び出し側で上限文字数(≈3000字)に切り詰め済み |
...
{"type":"summarize","instruction":"優先度順に最優先課題を課題キーで名指しし、ブロッカーと確認先を述べてください。","context":"PROJ-1 ログイン不具合 (期限超過3日 / 未割当) ...","lang":"ja"}
...
{"type":"summarize","text":"PROJ-1 が最優先です。期限を3日超過し未割当のため ..."}
