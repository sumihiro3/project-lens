...
- **主な機能**:
  ...
  - 優先対応リストの行クリック（`open-issue` / `show-background`）を `openInBacklog` で受け、選択中ワークスペースの `get_workspace_by_id` でドメインを取得し `https://{domain}/view/{issueKey}` を `@tauri-apps/plugin-shell` の `open` で開く（v0.4.6・IssueDetailDialog と同方式）
...
### `components/reports/CrossSummarySection.vue`（v0.4.5 新設 / v0.4.6 で大幅刷新・FR-V045-002 / FR-V046-001）

- **役割**: 複数プロジェクト横断サマリのプレゼンテーション専用セクション。決定的な「優先対応リスト」（`PriorityBlock`）を主役に、プロジェクト別統計テーブル（SQL 集計）と AI 構造化インサイト（`CrossInsightCard`）を表示し、再生成導線を提供する。v0.4.6 で `ReportNarrative` を廃止し `PriorityBlock` + `CrossInsightCard` に置き換え
- **Props**:
  - `stats: CrossSummaryStat[]` — プロジェクト別横断統計（未生成時は空配列）
  - `priorityList: PriorityList` — 優先対応リスト（`{ cross: PriorityIssue[], perProject: PriorityProjectGroup[] }` の**オブジェクト**。横断上位 + プロジェクト別。v0.4.6）
  - `insight: CrossInsight | null` — AI 構造化インサイト（`summary`/`recommendation` の2フィールド。v0.4.6。未生成・degrade 時は null）
  - `generatedAt: string | null` — 最終生成日時（ISO8601）
  - `loading: boolean` — 初期ロード中フラグ
  - `regenerating: boolean` — 再生成中フラグ（ボタンスピナー用）
  - `degradedReason: ReportDegradedReason | null` — インサイト非表示の degrade 理由
- **Emits**:
  - `regenerate: []` — 再生成ボタン押下（親が `generate_reports` を呼ぶ）
  - `select-project: [projectKey: string]` — プロジェクト行クリック（課題一覧の絞り込み導線）
  - `open-issue: [issueKey: string]` — 優先対応リストの行クリック（親が Backlog を開く。v0.4.6）
  - `show-background: [issueKey: string]` — 優先対応リストの背景要約導線（将来のボタン2分岐用にバブリング。v0.4.6）
- **主な機能**:
  - 優先対応リスト（統計テーブルの上に主役配置）: `PriorityBlock` に委譲。データ無し・未生成時はブロック非表示
  ...
  - AI 構造化インサイト（統計テーブルの下）: `CrossInsightCard` に委譲。インサイトなしは degrade 理由を `v-alert` で提示（`aiUnavailable`=warning / その他=info。NFR-V045-003）
- **使用コンポーネント**: `reports/PriorityBlock`, `reports/CrossInsightCard`
- **使用ユーティリティ**: `getProjectColor`, `formatDate`（`utils/issueHelpers`）
- **ステータス**: ✅ 実装済み（v0.4.5 / v0.4.6 で PriorityBlock+CrossInsightCard 構成へ刷新）
...
### `components/reports/PriorityBlock.vue`（v0.4.6 新設・FR-V046-001）

- **役割**: 優先対応リスト全体を管理するコンポーネント。横断上位 N=10 件のフラット一覧と、プロジェクト別上位 K=4 件のアコーディオンの2ブロックを `PriorityIssueList` で描画する。`CrossSummarySection` から `ReportNarrative` に代わる主役ブロックとして配置される
- **Props**:
  - `priorityList: PriorityList` — 優先対応リスト全体（`{ cross, perProject }` のオブジェクト。Rust `PriorityList` の camelCase 形）
- **Emits**:
  - `open-issue: [issueKey: string]` — 行クリック（`CrossSummarySection` → `reports.vue` へバブリング）
  - `show-background: [issueKey: string]` — 背景要約導線（将来のボタン2分岐用にバブリング）
- **主な機能**:
  - 「横断上位」フラット一覧（`PriorityIssueList`）: `priorityList.cross`（上位 N 件）をそのまま表示
  - 「プロジェクト別」アコーディオン（`v-expansion-panels` variant=accordion multiple）: `priorityList.perProject`（Rust が算出したプロジェクト別上位 K 件）を**そのまま**各パネルへ。UI 側で再グルーピングはしない（`cross` を regroup していた旧実装の取りこぼし＝プロジェクト別 top-K を失う、を避ける）
  - `cross`・`perProject` がともに空の場合は非表示（degrade 文言は `CrossSummarySection` 側）
- **使用コンポーネント**: `reports/PriorityIssueList`
- **ステータス**: ✅ 実装済み（v0.4.6）

### `components/reports/CrossInsightCard.vue`（v0.4.6 新設・FR-V046-001）

- **役割**: AI 構造化インサイト（概況・推奨アクション）をカード形式で表示するプレゼンテーション専用コンポーネント。生 AI テキストをそのまま出すのではなく、`CrossInsight`（`summary` / `recommendation`）の2フィールドを UI が整形して表示する。`CrossSummarySection` の統計テーブル下に配置される
- **Props**:
  - `insight: CrossInsight | null` — AI 構造化インサイト（`summary`=概況2〜3文、`recommendation`=推奨アクション1〜2文。未生成・degrade 時は null）
  - `degradedReason: ReportDegradedReason | null` — 非表示時の degrade 理由
- **主な機能**:
  - `insight` あり: `mdi-creation` + `ai.settings.generated` 生成ラベル + `summary`（概況）+ `recommendation`（推奨アクション）を `ai-text-box` スタイルのカードで表示
  - `insight` なし: `degradedReason` を `v-alert` で提示（`aiUnavailable`=warning / その他=info。NFR-V045-003）
- **使用ユーティリティ**: `useI18n`（`reports.cross.insight.*` キー）
- **ステータス**: ✅ 実装済み（v0.4.6）

### `components/reports/PriorityIssueList.vue`（v0.4.6 新設・FR-V046-001/002）

- **役割**: 決定的に算出された「優先対応リスト」を行リストで表示するプレゼンテーション専用コンポーネント。各行に課題キー・タイトル・優先理由チップ群・担当状況（連絡先表示）と Backlog 起票への導線を提供する。横断上位フラットとプロジェクト別グループの双方で使い回す
- **Props**:
  - `items: PriorityIssue[]` — 優先対応リスト（空配列なら何も描画しない。degrade 文言は親が出す）
  - `loading: boolean` — ロード中フラグ（スピナー表示用）
- **Emits**:
  - `open-issue: [issueKey: string]` — 行クリック（親が Backlog `https://{domain}/view/{issueKey}` を `@tauri-apps/plugin-shell` の `open` で開く。`IssueDetailDialog` と同方式）
  - `show-background: [issueKey: string]` — 背景要約への導線（将来のボタン2分岐用。当面は配線任意）
- **主な機能**:
  - 1段目: プロジェクトキー chip（`getProjectColor`）+ 課題キー + タイトル（`text-truncate`）
  - 2段目: 優先理由チップ群 + 担当状況。チップは `reasonStyle` で `PriorityReason` を i18n ラベル + 色分けにマップ（期限超過・リスク高=error / リスク中・停滞=warning / リスク低=success / 未割当=info / 担当者あり=default）
  - 担当状況は連絡先表示（あり=「{name} に確認」 / なし=「要アサイン」。未割当は info 色で強調）
  - `loading` 時はスピナー、`items` 空時は描画しない（NFR-V046-003 の degrade は親側）
- **使用ユーティリティ**: `getProjectColor`（`utils/issueHelpers`）, `useI18n`（理由・担当ラベル）
- **ステータス**: ✅ 実装済み（v0.4.6）
...
- **Export**:
  - `crossSummary`: 横断サマリ state（`stats` / `priorityList` / `insight` / `generatedAt`、Ref。`priorityList`・`insight` は v0.4.6 で `headline`/`narrative` を置き換え）
...
- **主な機能**:
  ...
  - `report_summaries.priority_json`（文字列）を `parsePriorityList` で `PriorityIssue[]` へパース（v0.4.6 追加。失敗時は空配列へ degrade）
  - `report_summaries.narrative`（横断時は `CrossInsightGeneration` JSON 文字列）を `parseCrossInsight` で `CrossInsight` へパース（v0.4.6 追加。失敗時は null へ degrade）
...
- **インターフェース**:
  ...
  - `ReportSummary`: `get_reports` / `generate_reports` の戻り値型（`workspaceId` / `reportType` / `periodKey` / `lang` / `statsJson` / `priorityJson` / `headline` / `narrative` / `generatedAt`）。`priorityJson` は v0.4.6 追加（優先対応リスト JSON 文字列・未算出は `null`）
  ...
  - `PriorityReason`: 優先対応理由の判別共用体（v0.4.6 追加。判別フィールドは **`type`**＝Rust serde tag と一致（`kind` ではない）。`{type:'overdue',days}` / `{type:'risk',level}` / `{type:'stale'}` / `{type:'unassigned'}` / `{type:'assignee',name}`）
  - `PriorityIssue` / `PriorityProjectGroup` / `PriorityList`: 優先対応リストの DTO（v0.4.6 追加。`PriorityIssue` = `issueKey` / `projectKey` / `title` / `assignee` / `reasons: PriorityReason[]`。`PriorityList` = `{ cross: PriorityIssue[], perProject: PriorityProjectGroup[] }`。`parsePriorityList` で `priority_json` 列の**オブジェクト** JSON からパース）
  - `CrossInsight`: AI 構造化インサイト（v0.4.6 追加。`summary`=概況2〜3文 / `recommendation`=推奨アクション1〜2文。`parseCrossInsight` で `narrative` 列の JSON 文字列からパース）
...
- **主なキー**:
  ...
  - `reports.priority.*`: 優先対応リストの理由・担当ラベル（v0.4.6 追加）
  - `reports.cross.insight.*`: AI 構造化インサイトのラベル（v0.4.6 追加）
- **ステータス**: ✅ 実装済み

### `locales/en.json`

- **役割**: 英語翻訳リソース
- **主なキー**: `ja.json`と同様（`dashboard.unknown` = "Unknown"。`reports.priority.*` / `reports.cross.insight.*` は v0.4.6 追加）
- **ステータス**: ✅ 実装済み
...
- **主な公開要素**:
  - `FoundationModelsBackend`: `LlmInference` + `EmbeddingBackend` 実装。`new(app)` で生成し、要求を内部の管理タスクへ MPSC 送信して oneshot で応答受信。`Clone` 可（同一管理タスク・同一状態を共有）
    ...
    - `summarize(&self, instruction, context, lang) -> Result<SummarizeOutput>`: 指示＋入力材料から **`@Generable` 構造化インサイト（`SummarizeOutput { summary, recommendation }`）** を生成（v0.4.6 / FR-V046-004。analyze/embed と同一 sidecar・同一管理タスクを経由。一時停止中は即エラー。AI 非対応・生成失敗は `Err` で呼び出し側が narrative を空へ degrade。`context` は呼び出し側で ≈3000字に切り詰め前提）
  ...
  - `SummarizeOutput`: summarize の構造化出力（v0.4.6。`summary`=概況 / `recommendation`=推奨アクション。`generate_cross_narrative` が JSON 化して `report_summaries.narrative` に保存）
...
- **テスト容易性**: sidecar 起動・通信を `SidecarTransport` / `SidecarProcess` トレイトで抽象化。本番は `ShellSidecarTransport`、テストはモックで管理タスクのロジック（analyze/embed/summarize の要求応答・再起動・連続失敗での一時停止・プロトコル整合・analyze と embed の sidecar 共用・embed の件数/次元検証・summarize の構造化応答/型不一致/error 応答/Suspended）を実機なしで検証（`#[cfg(test)]` で 21 テスト）
- **プロトコル整合**: `src-tauri/sidecar/` の入出力契約と一致（リクエスト `availability`/`analyze`/`summarize`/`embed`/`shutdown`、レスポンス `availability`/`result`/`summarize`/`embedding`/`error`）。embed の `prefix` は serde lowercase で `query`/`passage`（sidecar の `EmbedPrefix.rawValue` と一致）。`summarize`（**`@Generable` 構造化生成**・v0.4.6 / FR-V046-004）は Rust 側 `SidecarRequest::Summarize{instruction, context, lang}`（serde tag=type で `{"type":"summarize"}`）→ `SidecarResponse::Summarize{summary, recommendation}` を実装し `FoundationModelsBackend::summarize() -> Result<SummarizeOutput>` で公開（dispatch は `parse_summarize` ヘルパーを経由）。`context` は呼び出し側で ≈3000字に切り詰め前提
- **ステータス**: ✅ 実装済み（cargo build / clippy `-D warnings` / cargo fmt `--check` / 単体テスト 21件 通過。`tauri.conf.json` の `bundle.externalBin` 登録・build.sh での sidecar ビルド/署名/同梱は完了。実機での `externalBin` 起動連携・埋め込みモデル配置後の embed 応答・summarize の構造化インサイト出力品質は検証機での要確認）
...
- **入出力契約**（Rust 側 `ai/foundation_models.rs` `SidecarRequest`/`SidecarResponse` + `ai/mod.rs` + `ai/embedding.rs` と一致）:
  - リクエスト（1行 JSON）: `{type:"availability"}` / `{type:"analyze", issue_key, summary, description_head, status, due_date?, lang}` / `{type:"summarize", instruction, context?, lang}`（v0.4.6。`context` は呼び出し側で ≈3000字に切り詰め済み・FR-V046-003） / `{type:"embed", texts:[...], prefix:"query|passage"}`（v0.4。texts は切り詰め済み・**プレフィックス未付与**） / `{type:"shutdown"}`（EOF でも終了）
  - レスポンス（1行 JSON）: `{type:"availability", available, reason}`（reason: `available` / `appleIntelligenceNotEnabled` / `modelNotReady` / `deviceNotEligible` / `unavailableOther`） / `{type:"result", summary, risk_level, suggestion}` / `{type:"summarize", summary, recommendation}`（v0.4.6。`@Generable` 構造化インサイト・FR-V046-004） / `{type:"embedding", vectors:[[...512f...],...]}`（v0.4。入力 texts と同順・同数・512次元） / `{type:"error", message}`
...
- **Export**:
  ...
  - `generate_reports(app, workspace_id, report_type, lang) -> ReportSummary`: レポート/サマリーを生成して保存する Tauri コマンド（v0.4.5・FR-V045-002/003/006）。実体は **`pub(crate) generate_report(&app, &db, workspace_id, report_type, lang)`** に切り出し済みで、コマンドは薄いラッパー（`State<DbClient>` を `&DbClient` へ剥がして委譲）。`generate_report` をスケジューラの1日1回バックグラウンド生成（`scheduler::generate_due_reports`・FR-V045-005）と共有することで、手動再生成と自動生成が同一の生成経路を通る。`report_type`（`'cross_summary'`/`'weekly'`/`'monthly'`）に応じて統計を **SQL で決定的に集計**（横断=`get_cross_summary_stats`→`Vec<CrossSummaryStat>` を `stats_json` へ／週次月次=現在の期間境界で `get_period_activity_stats`→`Vec<PeriodActivityStat>` を `stats_json` へ）。**narrative の生成経路は種別で分岐（v0.4.6・FR-V046-004）**: 横断（`cross_summary`）は `collect_report_highlight_inputs`→`select_priority_list` で決定的な優先対応リスト（横断上位 N+プロジェクト別上位 K）を作り `PriorityList`（`{cross, perProject}`）を `serde_json` で `priority_json` に保存、`build_summarize_context`→`generate_cross_narrative`（新 `summarize` **`@Generable` 構造化生成**経路）で**概況・推奨アクションの構造化インサイト**を生成し JSON（`{summary, recommendation}`）化して `narrative` 列に保存（headline は空。AI の生テキストは画面に出さず UI が `CrossInsightCard` で整形表示）。週次/月次は従来どおり `select_report_highlights`→`build_report_context`→`generate_report_narrative`（`analyze` 流用）で headline/narrative を生成し `priority_json` は付けない。生成結果を `save_report_summary` で UPSERT（横断は `period_key='latest'`／週次月次は現在の期間キー）し、`get_report_summary` で読み戻して返す。横断の `me_user_id` は `get_workspaces` から当該 ws の `user_id` を解決。AI 非対応・narrative 失敗は `Err` にせず統計＋優先対応リストのみ保存し headline/narrative は `None`（degrade。NFR-V045-003/NFR-V046-003）。未知 `report_type` のみ `Err`
...
- **レポート生成コア（v0.4.5・FR-V045-002/003。`generate_reports` から利用される内部ヘルパー群）**:
  - 定数群（`SUMMARIZE_*` の隣）: `REPORT_HIGHLIGHT_MAX_ISSUES = 8`（注目上位件数の上限。目安5〜10の中庸）/ `REPORT_STALE_THRESHOLD_DAYS = 14`（停滞判定の未更新日数。`get_cross_summary_stats`/`get_report_highlight_inputs` と定義を揃える）/ `REPORT_CONTEXT_MAX_CHARS = SUMMARIZE_CONTEXT_MAX_CHARS`（≈3000。compact context の全体上限）/ `CROSS_SUMMARY_REGEN_HOURS = 20`（`pub(crate)`。横断サマリのバックグラウンド再生成の最小間隔。FR-V045-005。スケジューラ `cross_summary_is_due` が `report_summaries.cross_summary/latest` の `generated_at` 経過時間判定に参照）。**v0.4.6 追加（FR-V046-001/002）**: `REPORT_PRIORITY_CROSS_TOP_N = 10`（横断クロスプロジェクト優先対応リストの上位件数 N）/ `REPORT_PRIORITY_PROJECT_TOP_K = 4`（プロジェクト別グループの各プロジェクト上位件数 K）/ `REPORT_UNASSIGNED_SCORE = 8`（未割当課題への優先スコア従加点。「停滞+未割当」を上位化）。3 定数とも v0.4.6 で横断サマリ生成（`select_priority_list`→`priority_json` 保存）に結線済み
  ...
  - `collect_report_highlight_inputs(db, workspace_id) -> Vec<ReportHighlightInput>`: `get_report_highlight_inputs`（db）で通常課題のメタ（課題キー・タイトル・既存 ai_summary・risk・遅延日数・停滞・担当者・ステータス）を一括取得し `ReportHighlightInput` へ変換（プロジェクトキーは `project_key_from_issue_key` 導出・risk は `RiskLevel::from_storage_str`）。**新規 LLM 呼び出しゼロ**（v0.4.6 で title/assignee/status を追加・FR-V046-001）
  - `ReportType`（enum）: `CrossSummary` / `Weekly` / `Monthly`。`report_summaries.report_type` と一致し、narrative 生成指示文の言い回しを切り替える
  - `ReportHighlightInput`（struct）: 注目上位選定の入力1件分（`issue_key` / `project_key` / `title`（`issues.summary` = 課題タイトル）/ `ai_summary`（既存 `ai_results.summary` の1行要約）/ `risk_level`（`Option<RiskLevel>`）/ `delay_days`（SQL算出）/ `is_stale` / `assignee`（`Option<String>`。未割当は `None`）/ `status`（`Option<String>`。未設定は `None`））。DB/LLM 依存を持たず純粋関数で採点・連結できる（v0.4.6 で title/assignee/status を追加・FR-V046-001/002）
  - `report_highlight_score(item)`（純粋関数）: 重み付けスコア。**主**=期限超過日数（正の超過のみ・上限60日でクランプ）+ リスク（high=50/medium=25/low=5/未生成=0）、**従**=停滞（+10）+ 未割当（`REPORT_UNASSIGNED_SCORE = +8`。v0.4.6・FR-V046-002）の合算。未割当判定は `is_unassigned`（`None` または空白のみ）で一元化
  - `is_unassigned(assignee)`（純粋関数・v0.4.6）: 担当者が未割当（`None`／空白のみ）かを判定。`report_highlight_score` の従加点と `derive_priority_reasons` の「要アサイン/～に確認」分岐を一元化
  ...
  - **優先対応リスト（v0.4.6・FR-V046-001/002。横断サマリ生成（`generate_report` の `cross_summary` 分岐）から結線済み）**:
    - `PriorityReason`（enum・`#[serde(tag="type", rename_all="camelCase")]`）: 4 シグナルを表す判別共用体。`Overdue { days }`（期限超過・遅延日数）/ `Risk { level }`（内容リスク high/medium。low は理由化しない）/ `Stale`（停滞。日数は持たず理由ラベルで吸収）/ `Unassigned`（要アサイン）/ `Assignee { name }`（～に確認）。フロントは `type` をキーに i18n ラベル・色分けを引く
    - `derive_priority_reasons(item) -> Vec<PriorityReason>`（純粋関数）: 1課題のメタから優先理由を導出。出力順は **期限超過→リスク→停滞→担当状況**。担当状況（`Unassigned`/`Assignee`）は常に1件付くため最低1件
    - `PriorityIssue`（struct・`#[serde(rename_all="camelCase")]`）: UI へ渡す優先対応リスト1行の決定的 DTO（`issueKey` / `projectKey` / `title` / `assignee`（空白のみは `None` 正規化） / `reasons: Vec<PriorityReason>`）。LLM 非依存。`from_input(&ReportHighlightInput)` で組み立て
    - `select_priority_list(items) -> (Vec<PriorityIssue>, Vec<(String, Vec<PriorityIssue>)>)`（純粋関数）: 理由が空でない課題をスコア降順に整列し、**横断**=上位 `REPORT_PRIORITY_CROSS_TOP_N` 件のフラットリスト、**プロジェクト別**=プロジェクトキーごとに各上位 `REPORT_PRIORITY_PROJECT_TOP_K` 件（グループ並びは代表＝最上位スコア降順）を返す。横断とプロジェクト別は同一母集団から独立に切り出すため同じ課題が両ブロックに現れうる（UI のフラット上位+アコーディオン2ブロック併存を意図）
    - `PriorityProjectGroup<'a>` / `PriorityList<'a>`（serde DTO・`#[serde(rename_all="camelCase")]`・v0.4.6）: `priority_json` のシリアライズ用ラッパー。`select_priority_list` の `(cross, per_project)` を `PriorityList::new(&cross, &per_project)` で `{ cross: [PriorityIssue], perProject: [{ projectKey, issues:[PriorityIssue] }] }` の camelCase 形へ整形（タプル配列の位置依存形を避けフロントが扱いやすいキー付きオブジェクトへ正規化）。`report_summaries.priority_json` に保存され `useReports` がパースして横断フラット+プロジェクト別アコーディオンを描画
    - `build_summarize_context(cross, per_project, lang) -> String`（純粋関数・v0.4.6・FR-V046-004）: 横断優先対応リスト（課題キー+タイトル+優先理由+担当）+プロジェクト別件数を compact text 化し `SUMMARIZE_CONTEXT_MAX_CHARS`（≈3000）で切り詰め。優先理由は内部ヘルパー `format_priority_reasons(reasons, lang)` で**出力言語にローカライズ**して整形（ja=`期限超過N日`/`高リスク`/`停滞`/`未割当`・en=`overdue Nd` 等）。小型モデルが context の語を写すため、英語の機械ラベルを渡すと日本語出力に英語が混入するのを防ぐ。見出し・未割当ラベルも `lang` に合わせる。横断リスト空なら空文字。**新規 per-issue LLM 呼び出しゼロ**（決定的算出済みの優先対応リストのみを材料にする）
    - `generate_cross_narrative(app, context, lang) -> String`（v0.4.6・FR-V046-004/005）: 横断サマリのインサイトを **`summarize` 構造化生成**経路で作り、`{summary, recommendation}` の JSON 文字列で返す（`narrative` 列に保存）。`FoundationModelsBackend::new(app)` を直接生成し `summarize(instruction, context, lang) -> SummarizeOutput` を呼ぶ（`summarize` は具体型の inherent メソッドのため `create_backend` の `impl LlmInference` 越しには呼べない）。instruction は「**一般論でなく具体的に**・最優先を課題キーで名指し・超過日数などの数値・横断の傾向（同種の長期放置の集中等）・着手/アサイン先を述べる」を ja/en で用意（フィールドの役割・Markdown 禁止は sidecar の `@Guide` が担う）。`summary`/`recommendation` は `sanitize_narrative` で Markdown 除去。context 空・AI 非対応・生成失敗は `Err` にせず**空文字**へ degrade（NFR-V046-003。優先対応リスト・統計は維持）
  - `build_report_context(items)`（純粋関数）: 注目上位群を「`[project] issue_key title / overdue Nd / risk R / stale / assignee:A / status:S` + 既存 ai_summary」の1〜2行に詰めて連結し `REPORT_CONTEXT_MAX_CHARS` で切り詰め。担当者未割当は `assignee:unassigned` として明示。**新規 per-issue LLM 呼び出しゼロ**で既存 `ai_results` を再利用（NFR-V045-002。v0.4.6 で title/assignee/status を context に追加）。**週次/月次レポート専用**（横断は v0.4.6 で `build_summarize_context` へ置換）
  - `generate_report_narrative(app, context, lang, report_type) -> (headline, narrative)`: `summarize_solutions` と同様 **sidecar 改修なしで既存 `analyze` 経路を流用**。context を `AiAnalysisInput.description_head` に、`report_type`/`lang` 別の指示文を `summary` に載せ `create_backend`（FoundationModels 再利用）→`infer`。`output.summary`→headline（見出し1行）/ `output.suggestion`→narrative（注目点）にマップ。context 空・AI 非対応・生成失敗は `Err` にせず空タプル `(String::new(), String::new())` へ degrade（NFR-V045-003）。**週次/月次レポート専用**（横断は v0.4.6 で `generate_cross_narrative`（`summarize` 経路）へ置換。旧経路コードは週次/月次用に保持）
- **ステータス**: ✅ 実装済み（cargo build / clippy `--all-targets -D warnings` / 単体テスト通過。v0.3 の AI コマンド5種に加え、v0.4 の類似検索・解決策要約コマンド4種を追加・`lib.rs` の invoke_handler に登録。`rank_similar`/`project_key_from_issue_key`/`build_solution_context` の単体テスト10件。v0.4.5 でレポート生成コア（定数4 / `ReportType` / `ReportHighlightInput` / `report_highlight_score` / `select_report_highlights` / `build_report_context` / `generate_report_narrative`）と期間キーヘルパー（`iso_week_key`/`iso_week_bounds`/`month_key`/`month_bounds`/`date_to_utc_midnight`）・生成コマンド3種（`generate_reports`/`get_reports`/`list_report_periods`）を追加し `lib.rs` に登録。重み付け・N件クランプ・超過60日クランプ・context 包含/文字数上限・空入力に加え、ISO 週番号/年境界(2027-01-01→2026-W53)/週月境界(月曜〜翌月曜・当月1日〜翌月1日・12月の年繰り上げ)の単体テスト。v0.4.6 で優先対応リスト（`PriorityReason`/`PriorityIssue`/`derive_priority_reasons`/`select_priority_list`/`is_unassigned` と定数3 `REPORT_PRIORITY_CROSS_TOP_N`/`REPORT_PRIORITY_PROJECT_TOP_K`/`REPORT_UNASSIGNED_SCORE`）を追加し、横断サマリ narrative を `summarize` **`@Generable` 構造化生成**経路へ置換（`build_summarize_context`/`format_priority_reasons`/`generate_cross_narrative`/`sanitize_narrative`/`normalized_assignee`/`PriorityList`/`PriorityProjectGroup`/`CrossInsightJson` を追加。`generate_report` の `cross_summary` 分岐で `select_priority_list`→`priority_json` 保存＋構造化インサイト（概況/推奨）の JSON を `narrative` に保存。週次/月次は `build_report_context`+`generate_report_narrative` の `analyze` 流用を据え置き）。これらの結線で v0.4.6 優先対応リストの該当シンボルは `#[allow(dead_code)]` を解除。未割当加点・4シグナル理由導出（low非理由化・担当状況は常に付与）・横断N件クランプ・プロジェクト別K件グルーピング/切り詰め・空白担当のNone正規化に加え summarize context 包含/文字数上限/空入力・`priority_json` の camelCase 形の単体テストを追加して commands::tests 33件。さらに v0.4.5 課題背景要約コマンド `get_background_summary`（定数 `BACKGROUND_SUMMARY_COMMENTS_MAX_CHARS = 2000`・コメント `source_hash` キャッシュ付き）を追加し `lib.rs` に登録（`compute_source_hash` は埋め込みと共用するため `pub(crate)` 化済み）。v0.4.5 スケジューラ結線（FR-V045-005）に向けて `generate_reports` の生成コアを `pub(crate) generate_report(&app, &db, …)` へ抽出し、`CROSS_SUMMARY_REGEN_HOURS`・`iso_week_key`・`month_key` を `pub(crate)` 化して `scheduler::generate_due_reports` から共有（コマンドと自動生成が同一経路）。1日1回バックグラウンド自動生成の判定・実行は `scheduler.rs` 側に実装済み）
...
- **主な構造体**:
  ...
  - `ReportSummary`（v0.4.5 新設・v0.4.6 拡張）: `report_summaries` テーブル1行。横断サマリ・週次/月次レポートの統計 JSON・AI narrative・見出しを保持。`report_type`('cross_summary'/'weekly'/'monthly') / `period_key`(横断='latest'・週次='YYYY-Www'・月次='YYYY-MM') / `lang` / `stats_json` / `headline` / `narrative` / `priority_json`（v0.4.6: 優先対応リスト JSON・未算出は `None`） / `generated_at`
...
- **主なテーブル**:
  ...
  - `report_summaries`（v0.4.5 新設・v0.4.6 拡張）: レポート/サマリー保存。PK は `(workspace_id, report_type, period_key, lang)`。`stats_json`（プロジェクト別集計 JSON）/ `headline`（AI 見出し）/ `narrative`（AI narrative）/ `priority_json`（v0.4.6: 優先対応リスト JSON・非破壊 ALTER で追加）/ `generated_at`。横断サマリは `period_key='latest'` で最新上書き、週次/月次は期間キーで履歴保持（FR-V045-006）
...
- **主な機能**:
  ...
  - レポート/サマリー操作（v0.4.5・FR-V045-006/003・v0.4.6 拡張）: `save_report_summary(ws, report_type, period_key, lang, stats_json, headline, narrative, priority_json)`（`report_summaries` を `INSERT OR REPLACE` で UPSERT・`generated_at` は now 自動設定・横断は `period_key='latest'` で上書き／週次月次は期間キーで履歴保持。narrative=`None`/priority_json=`None` の degrade 保存可） / `get_report_summary(ws, report_type, period_key, lang)`（`ReportSummary` を1行取得・`priority_json` 列含む。未生成は `None`） / `list_report_periods(ws, report_type)`（DISTINCT な `period_key` を `MAX(generated_at)` 降順で返す期間セレクタ用。同一期間に複数言語があっても重複しない）
  ...
  - レポート決定的集計（v0.4.5・FR-V045-002/003 / v0.4.6 拡張・FR-V046-001）: `get_cross_summary_stats(ws, me_user_id, stale_threshold_days)`（横断サマリ。通常課題（`is_corpus_only=0`）をプロジェクト別に集計し `Vec<CrossSummaryStat>` を返す。期限超過＝`due_date < 今日`・停滞＝`updated_at` が `stale_threshold_days` 日以上前・自分担当の要対応＝担当者が `me_user_id`（raw_data の `assignee.id` を `json_extract`）かつ期限超過 or 停滞・risk 分布は `ai_results` を LEFT JOIN。日付判定は `julianday(substr(...,1,10))` で統一。プロジェクトキー導出は SQL では難しいため課題1行のフラグを SQL 算出→Rust で `commands::project_key_from_issue_key` 相当により集約） / `get_period_activity_stats(ws, period_start, period_end)`（週次/月次。半開区間 `[start, end)` の文字列辞書順比較で作成/更新/完了を判定し `Vec<PeriodActivityStat>` を返す。完了は `is_corpus_only=1` かつ更新が期間内・期間内アクティビティが無い課題は行を返さない） / `get_report_highlight_inputs(ws, stale_threshold_days)`（注目上位選定用。通常課題（`is_corpus_only=0`）の `(issue_key, title, ai_summary, risk_level, delay_days, is_stale, assignee, status)` を1クエリで返す。`title` は `issues.summary`（課題タイトル）。`ai_results` を LEFT JOIN し既存 `summary`/`risk_level` を再利用＝**新規 LLM 呼び出しゼロ**・遅延日数は julianday 差を Rust 側で符号反転・停滞は julianday 比較。`assignee`/`status` は NULL 許容。`commands::collect_report_highlight_inputs` が `ReportHighlightInput` へ変換。v0.4.6 で title/assignee/status を追加）。停滞しきい値・期間境界は呼び出し側（scheduler/commands の定数）で決め、メソッドは引数で受ける
...
- **ステータス**: ✅ 実装済み（v0.4 DBスキーマ拡張＋スケジューラ結線対応完了。v0.4.5 で `report_summaries` / `issue_background_summary` テーブル新設・`ReportSummary`（`#[serde(rename_all = "camelCase")]`） / `IssueBackgroundSummary` 構造体追加・`delete_workspace` / `delete_workspace_issues` / `save_issues` の孤児掃除に両テーブルを追加・レポート/背景要約の CRUD メソッド5件（`save_report_summary` / `get_report_summary` / `list_report_periods` / `save_background_summary` / `get_background_summary`）を追加。さらに決定的集計メソッド2件（`get_cross_summary_stats` / `get_period_activity_stats`）と集計結果構造体 `CrossSummaryStat` / `PeriodActivityStat`（serde camelCase）を追加・`issues.created_at` カラム（非破壊 ALTER）と `save_issues` への `created_at` 展開・`Issue.created`（backlog.rs）取り込み・`commands::project_key_from_issue_key` を `pub(crate)` 化。v0.4.6 で `report_summaries.priority_json` カラム（非破壊 ALTER）を追加・`ReportSummary` 構造体に `priority_json: Option<String>` フィールドを追加・`save_report_summary` の引数に `priority_json` を追加・`get_report_summary` の SELECT 列に `priority_json` を追加。`cargo build` / `clippy -D warnings` / `fmt --check` / 単体テスト 89件通過）
...
- 2026-06-14: v0.4.6 DB 拡張 — `get_report_highlight_inputs` に title/assignee/status を追加(db.rs・commands.rs・FR-V046-001)。`db.rs::get_report_highlight_inputs` の SELECT に `i.summary AS title`・`i.assignee`・`i.status` を追加し、戻り値タプルを `(issue_key, title, ai_summary, risk_level, delay_days, is_stale, assignee, status)` へ拡張。`commands.rs::ReportHighlightInput` 構造体に `title: String`・`assignee: Option<String>`・`status: Option<String>` の3フィールドを追加。`collect_report_highlight_inputs` のタプル分解・構築を対応。`build_report_context` の context 行に `title`・`assignee`(未割当は `assignee:unassigned`)・`status` を追加し narrative の名指し精度を向上。テストヘルパー `hl()` に `title`/`assignee=None`/`status=None` を追加。`cargo build` / `clippy -D warnings` / 単体テスト 104件通過
- 2026-06-14: v0.4.6 横断サマリ narrative を summarize 経路へ置換し優先リストを生成・保存(commands.rs・FR-V046-004/005)。`generate_report` の `cross_summary` 分岐のみを新実装へ置換(週次/月次は `build_report_context`+`generate_report_narrative` の `analyze` 流用を据え置き)。新設の純粋関数 `build_summarize_context(cross, per_project)`(優先対応リスト=課題キー+タイトル+優先理由+担当 と プロジェクト別件数を compact text 化し `SUMMARIZE_CONTEXT_MAX_CHARS=3000` で切り詰め。理由整形は内部ヘルパー `format_priority_reasons_compact`)と `generate_cross_narrative(app, context, lang)`(`FoundationModelsBackend::new` を直接生成し新 `summarize` 自由文経路を呼ぶ。instruction は「最優先数件を課題キーで名指し・ブロッカー・誰に確認/一般論回避/事実のみ」を ja/en で用意。context 空・AI 非対応・生成失敗は空文字へ degrade)を追加。serde DTO `PriorityProjectGroup<'a>`/`PriorityList<'a>`(`PriorityList::new` で `(cross, per_project)` を `{cross, perProject:[{projectKey, issues}]}` の camelCase へ整形)を追加し `priority_json` に保存。`cross_summary` 分岐は `collect_report_highlight_inputs`→`select_priority_list`→`PriorityList::new`+`serde_json` で `priority_json` 化、`build_summarize_context`→`generate_cross_narrative` で名指し narrative を生成、`save_report_summary` に `priority_json` を渡して保存(headline は当面空)。結線により v0.4.6 優先対応リストの該当シンボル(`PriorityReason`/`PriorityIssue`/`derive_priority_reasons`/`select_priority_list`/`from_input` と定数 `REPORT_PRIORITY_PROJECT_TOP_K`)から `#[allow(dead_code)]` を解除。`summarize` context 包含/文字数上限/空入力・`priority_json` の camelCase 形の単体テスト4件を追加(commands::tests 33件)。`cargo build` / `cargo clippy -D warnings` / 単体テスト(commands::tests 33件) 通過(注: `foundation_models` の `summarize_suspends_after_consecutive_failures` は並列実行時のみ稀に共有モック状態で flaky・本作業と無関係。単独実行・連続フル実行では安定通過)
- 2026-06-14: v0.4.6 sidecar summarize 経路の Rust 側結線(foundation_models.rs・FR-V046-003)。Swift sidecar の自由文生成プロトコルに対応する Rust 側を実装し、これまで「sidecar 側のみ実装済み」だった `summarize` を Rust から呼べるようにした。`SidecarRequest` に `Summarize{instruction, context, lang}`(serde tag=type で `{"type":"summarize"}` へ自動マップ)・`SidecarResponse` に `Summarize{text}` バリアントを追加。`ManagerCommand::Summarize{instruction, context, lang, respond: oneshot<Result<String>>}`・`ExpectedResponse::Summarize` を追加し、`dispatch_command` に Summarize アーム(`exchange`→新設ヘルパー `parse_summarize`)・`fail_command` に Summarize アームを追加。`parse_result`/`parse_availability`/`parse_embedding` の網羅 match に新バリアントの型不一致アームを追加(match 漏れなし)。公開 API として `FoundationModelsBackend::summarize(&self, instruction, context, lang) -> Result<String>` を `embed_internal` と同じ oneshot/Suspended パターンで実装(analyze/embed と同一 sidecar・同一管理タスクを共用)。AI 非対応・生成失敗は `Err` で呼び出し側が narrative を空へ degrade(FR-V046-005)。`context` の ≈3000字切り詰めは呼び出し側責務。モック transport で summarize の自由文応答・要求シリアライズ契約・型不一致(`parse_summarize`)・error 応答(再起動しない)・連続失敗での Suspended を検証する単体テスト5件を追加(foundation_models 計21件)。`cargo build` / `clippy -D warnings` / `cargo fmt --check`(本ファイル) / 単体テスト 通過
- 2026-06-14: v0.4.6 リリース検査(release-check 相当)。全検査グリーン確認。pnpm lint: 0 errors (1 warning: issueHelpers.ts の `@typescript-eslint/no-explicit-any` は既存・エラーなし)。format:check: docs/releases/v0.4.6/requirements.md と docs/REQUIREMENTS.md の Prettier 違反を `prettier --write` で修正後グリーン。clippy (-D warnings): Finished (警告なし)。format:rust:check: グリーン。generate: 7ルート prerender 成功。sidecar: `swift build -c release` 成功 (Swift 6.3 / macOS 26 SDK)・`binaries/projectlens-ai-sidecar-aarch64-apple-darwin` へ再配置・アドホック codesign 完了 (NFR-V046-004)。目視確認(tauri:dev)は AI 非対応機でのスモーク確認のため本項対象外。
- 2026-06-14: v0.4.6 ドキュメント同期。新規コンポーネント `PriorityBlock.vue`（優先対応リスト全体・横断フラット+プロジェクト別アコーディオン・`PriorityIssueList` を2ブロックで使用）と `CrossInsightCard.vue`（AI 構造化インサイト・`summary`/`recommendation` の2フィールドをカード表示）を追加。`CrossSummarySection.vue` の使用コンポーネントを `ReportNarrative`→`PriorityBlock`+`CrossInsightCard` に更新し Props を `headline`/`narrative`→`insight: CrossInsight | null` に変更。`useReports.ts` の `crossSummary` state を `priorityList`/`insight` に更新し `CrossInsight` 型・`parseCrossInsight()` を追加。`locales/{ja,en}.json` に `reports.priority.*`・`reports.cross.insight.*` キーを追記。REQUIREMENTS.md の v0.4.6 ステータスを実装済みに更新
- 2026-06-14: v0.4.6 narrative を**自由文生成 → `@Generable` 構造化生成へ作り替え**（実機フィードバック「生 AI テキストがエコー/英語混入/名前崩れ/Markdown 化・浅い」への対応）。sidecar に `@Generable struct CrossInsightGeneration { summary, recommendation }`（@Guide で具体性＝最優先の課題キー名指し・数値・傾向を要求、全件列挙は禁止）を追加し応答を `{type:"summarize", summary, recommendation}` へ。Rust `SidecarResponse::Summarize{summary, recommendation}`・`SummarizeOutput`・`summarize()->Result<SummarizeOutput>`・`parse_summarize`、`generate_cross_narrative` は `{summary, recommendation}` を JSON 化して `narrative` に保存（`sanitize_narrative` で Markdown 除去・保険）、`build_summarize_context` を `lang` 対応にしラベルをローカライズ。フロントは `CrossInsight` 型・`parseCrossInsight`・`CrossInsightCard.vue` で生テキストを出さずカード整形。実機（macOS 26.3.1）の sidecar スモークで構造化・日本語・名前正確・具体的洞察を確認。COMPONENTS.md の summarize 契約記述（`text`/`Result<String>`/「自由文」）を構造化版へ同期
- 2026-06-14: v0.4.6 再レビュー(review-changes 2回目)の修正。バグ:ランタイム契約バグはゼロ（構造化 summarize の Swift↔Rust↔TS は一致を確認）。規約/品質: main.swift ヘッダの summarize 出力契約コメントを構造化版へ修正、`is_unassigned` を `normalized_assignee`（担当者正規化の単一真実源）に一元化し `derive_priority_reasons`/`PriorityIssue::from_input` も共用（インライン重複を排除）、`reports.vue` の `openInBacklog` を毎クリック `get_workspace_by_id` から `workspaceDomains` キャッシュ参照へ。COMPONENTS.md の旧記述（`priorityList: PriorityIssue[]`→`PriorityList`、`PriorityReason` の `kind`→`type`、`build_summarize_context` 署名/`format_priority_reasons` 名）を実装へ同期
