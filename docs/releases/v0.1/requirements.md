# ProjectLens - 要件定義書(v0.1 ベースライン)

> **リリース**: v0.1(初期リリース。2025-11 時点の実装済み機能のベースライン)
> **ステータス**: 確定(実装済み)
...
- **優先度**: 必須
- **説明**: 登録済みワークスペースの有効・無効を切り替える
- **受け入れ基準**:
  - 設定画面でのトグルスイッチによる切り替え
  - 無効化時の課題データの非表示・削除
  - 有効化時の課題データの再同期
...
- **優先度**: 推奨
- **説明**: アプリケーションの状態を確認・管理する
- **受け入れ基準**:
  - Backlog API使用状況（残り回数・リミット・リセット時刻）の表示
  - ログファイル保存場所の確認とオープン機能
