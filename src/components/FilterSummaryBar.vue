...
const filterSummary = computed(() => {
  ...
  // ステータス
  const statusOption = statusFilterOptions.value.find(
    opt => opt.value === props.filters.statusFilter
  )
  ...
  // 期限
  const dueDateOption = dueDateFilterOptions.value.find(
    opt => opt.value === props.filters.dueDateFilter
  )
  if (dueDateOption) {
  ...
  if (props.filters.selectedPriorities.length > 0) {
    parts.push(
      t('filters.summary.priority', { value: props.filters.selectedPriorities.join(', ') })
    )
  }
